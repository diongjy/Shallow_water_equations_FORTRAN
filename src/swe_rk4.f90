! Solver module Runge-Kutta scheme
...
subroutine rk4 (ts)
  ...
  !Call tendencies - RK4 step2 of 4
  call solve(ku2, kv2, kh2, uo+ku1*dt2, vo+kv1*dt2, ho+kh1*dt2)
  !Call tendencies - RK4 step3 of 4
  call solve(ku3, kv3, kh3, uo+ku2*dt2, vo+kv2*dt2, ho+kh2*dt2)
 ! Call tendencies - RK4 step4 of 4
  call solve(ku4, kv4, kh4, uo+ku3*dt2, vo+kv3*dt2, ho+kh3*dt2)   
...
subroutine solve (uout, vout, hout, uin, vin, hin)
    ...
    !Constant, distance variables for gradien calculation
    ...
    !diffusion multiplier constans for diffusion terms
    ...
    !gravity/pressure gradient force in momentum equations
    real(8) :: gx, gy
   ...
   do j2=1, ny
      ...
      do i2= 1, nx
         ...
         ! vx term in y-momentum equation
         vx = uin(xc,yc)*(vin(xf,yc) - vin(xb,yc))*dx20 

         ! vy term in y-momentum equation
         ...
         ! vxx diffusion term in v-momentum equation
         vxx = (vin(xf,yc) - 2.0*vin(xc,yc) + vin(xb,yc))*nudxx

         ! vyy diffusion term in y-momentum equation
